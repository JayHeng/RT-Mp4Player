...
### 1 硬件连接
#### 1.1 开发板
　　本应用基于的开发板是恩智浦 XRT1060EVK12，主芯片为i.MXRT1062DVJ6A，主频为600MHz，内置1MB SRAM（最大512KB TCM），开发板原理简图如下：  
...
##### 2.2.2 一些重要的配置宏
　　工程重要的配置宏在 \RT-MPlayer\boards\evkmimxrt1060\demo_apps\mp4\mp4.h中：  
...
### 3 其他功能
#### 3.1 统计FFmpeg解码时间
　　在 \RT-MPlayer\boards\evkmimxrt1060\demo_apps\mp4\mp4.h 中开启下面2个宏，编译应用工程执行后会在SD卡中生成time.txt文件，该文件保存了ffmpeg解码以及PXP,LCD耗时数据（默认是统计2000帧音视频数据，如需修改请在工程中搜索 FF_MEASURE_FRAMES）。  
...
　　使用 \RT-MPlayer\bin\count_ffmpeg_time.py 脚本可以读取time.txt中的数据并用图表显示出来，脚本命令如下（其中xxResoultion, xxBuild参数仅是用于在图表中显示标题信息）：  

> jay@pc /d/RT-MPlayer/bin
