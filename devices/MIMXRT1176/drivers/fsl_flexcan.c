...
void FLEXCAN_Init(CAN_Type *base, const flexcan_config_t *config, uint32_t sourceClock_Hz)
{
    ...
    /* Enable to write MCCR. */
    uint32_t tempMECR = base->MECR;
    if ((tempMECR & CAN_MECR_ECRWRDIS_MASK))
    {
        tempMECR &= ~CAN_MECR_ECRWRDIS_MASK;
        base->MECR = tempMECR;
    }
#endif
...
#if (defined(FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER)) && (FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER > 0)
        if ((0U == (base->IFLAG1 & base->IMASK1)) && (0U == (base->IFLAG2 & base->IMASK2)))
#else
        if (0U == (base->IFLAG1 & base->IMASK1))
#endif
        {
            return false;
        }
        ...
        if (((uint8_t)kFLEXCAN_StateIdle != handle->rxFifoState) && (result <= (uint32_t)CAN_IFLAG1_BUF7I_SHIFT))
        {
            ...
            switch (u32mask << result)
            {
                ...
                case (int32_t)kFLEXCAN_RxFifoFrameAvlFlag:
                    if (kStatus_Success == (status = FLEXCAN_ReadRxFifo(base, handle->rxFifoFrameBuf)))
                    {
