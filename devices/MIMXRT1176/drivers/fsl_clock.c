...
void CLOCK_InitArmPll(const clock_arm_pll_config_t *config)
{
    ...
    if ((ANADIG_PLL->PLL_ARM_CTRL & (ANADIG_PLL_PLL_ARM_CTRL_POWERUP_MASK))
            && ((ANADIG_PLL->PLL_ARM_CTRL & (ANADIG_PLL_PLL_ARM_CTRL_DIV_SELECT(config->loopDivider))) == (ANADIG_PLL_PLL_ARM_CTRL_DIV_SELECT(config->loopDivider)))
            && ((ANADIG_PLL->PLL_ARM_CTRL & (ANADIG_PLL_PLL_ARM_CTRL_POST_DIV_SEL(config->postDivider))) == (ANADIG_PLL_PLL_ARM_CTRL_POST_DIV_SEL(config->postDivider))))
    {
...
void CLOCK_InitSysPll3(const clock_sys_pll3_config_t *config)
{
    ...
    if ((ANADIG_PLL->PLL_480_CTRL & ANADIG_PLL_PLL_480_CTRL_POWERUP_MASK)
            && ((ANADIG_PLL->PLL_480_CTRL & ANADIG_PLL_PLL_480_CTRL_DIV_SELECT(config->divSelect)) == ANADIG_PLL_PLL_480_CTRL_DIV_SELECT(config->divSelect)))
    {
        /* no need to reconfigure the PLL if all the configuration is the same */
